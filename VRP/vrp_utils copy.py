...
def create_VRP_dataset(
    ...
    data=np.load("data/output_3d.npy", allow_pickle=True) 
...
class DataGenerator(object):
    def __init__(self, 
        ...
        self.data = create_VRP_dataset()

        self.test_data = self.data[-self.n_problems:]
        self.count_train=0
    ...
    def get_train_next(self):
        ...
        input_data = self.data[self.count_train:self.count_train+self.args['batch_size'],:,:]
        self.count_train+=self.args['batch_size']
...
class Env(object):
    def __init__(self,
        ...
        '''
        self.num_of_orders=args['num_of_orders']
        self.num_unique_orders=args['num_unique_orders']
        self.num_production_lines=args['num_production_lines']
        self.labour_max=args['labour_max']
        
        self.input_data = tf.compat.v1.placeholder(tf.float32,\
            shape=[None,self.num_of_orders,self.num_unique_orders+self.num_production_lines])
        
        print("shape=",self.input_data.shape)

        self.production_times = self.input_data[:,:,:self.num_production_lines]
        self.changeover_times = self.input_data[:,:,self.num_production_lines:]
        
        self.batch_size = tf.shape(self.production_times)[0] 
...
#         self.input_pnt = tf.tile(self.input_pnt, [self.beam_width,1,1])
        ...
        self.changeover_times = tf.tile(self.changeover_times, [self.beam_width,1])
        self.production_times = tf.tile(self.production_times, [self.beam_width,1])
        ...
        state = State(production_times = self.production_times,
                    changeover_times = self.changeover_times,
                    num_of_orders = self.num_of_orders,
                    num_unique_orders = self.num_unique_orders,
                    labour_max = self.labour_max,
                    )
    ...
    def step(self,
        ...
        if beam_parent is not None:
            ...
            self.changeover_times= tf.gather_nd(self.changeover_times,batchedBeamIdx)
            self.production_times= tf.gather_nd(self.production_times,batchedBeamIdx)
            #load:[batch_size*beam_width]
